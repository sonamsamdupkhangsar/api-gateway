...
RUN --mount=type=secret,id=USERNAME --mount=type=secret,id=PERSONAL_ACCESS_TOKEN --mount=type=cache,target=/root/.gradle\
    ...
    export PERSONAL_ACCESS_TOKEN=$(cat /run/secrets/PERSONAL_ACCESS_TOKEN) &&\
     ./gradlew clean build
RUN  mkdir -p build/dependency && (cd build/dependency; jar -xf ../libs/api-gateway-1.0-SNAPSHOT.jar)
...
ENTRYPOINT ["java", "-javaagent:app/newrelic/newrelic.jar", "-cp","app:app/lib/*","me.sonam.apigateway.ApiGatewayApplication"]
